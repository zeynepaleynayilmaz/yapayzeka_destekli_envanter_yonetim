...
export function UrunSec({ modalDurumu, setModalDurumu, setSelectedProduct}) {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      if (secilmisUrunModeli && secilmisAltKategori && secilmisUrunTuru ) {
        axios
          .get("http://localhost:3000/sales-data", {
            params: { urunId: secilmisUrunModeli.id },
          })
          .then((response) => {
            setSelectedProduct(response.data); // Seçilen ürün verisini setle
          });
      }
    }, [secilmisUrunModeli]);
    ...
    <Modal open={modalDurumu} onClose={modaliKapat}>
      ...
      >
        <Typography variant="h6" component="h2">
          <CameraAltIcon sx={{ mr: 1 }} /> Ürün Seç
        </Typography>
        ...
        <Button
          ...
          fullWidth
          onClick={() => {
            if (secilmisUrunModeli) {
              setSelectedProduct(secilmisUrunModeli); // Seçilen ürünü gönder
              setModalDurumu(false); // Modalı kapat
            } else {
              alert("Lütfen bir ürün modeli seçiniz!");
            }
          }}
        >
          Ürünü Getir
