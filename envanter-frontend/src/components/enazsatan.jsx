...
import { EnCokSatanChart } from "./encoksatan";

Chart.register(...registerables);

export const EnAzSatanChart = () => {
  const [products, setProducts] = useState([]);
  const [year, setYear] = useState(2024);
  const [month, setMonth] = useState(new Date().getMonth() + 1);
  const chartRef = useRef(null);
  const chartInstanceRef = useRef(null);
  const [showEnCokSatan, setShowEnCokSatan] = useState(false);

  const handleClick = () => {
    setShowEnCokSatan(true);
  };

  // Veri çekme fonksiyonu
  const fetchData = (selectedYear, selectedMonth) => {
    fetch(
      `http://localhost:3000/enaz-satan?year=${selectedYear}&month=${selectedMonth}`
    )
  ...
  useEffect(() => {
    ...
    if (products.length > 0 && !showEnCokSatan) {
  ...
  }, [products, showEnCokSatan]);

  if (showEnCokSatan) {
    return <EnCokSatanChart />;
  }
